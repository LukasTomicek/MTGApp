...
class BuyListViewModel(
...
) : BaseViewModel<BuyListScreenState, BuyListUiEvent, BuyListDirection>(
    initialState = BuyListScreenState(),
) {
                ...
                if (user == null) {
                    ...
                    updateState {
                        it.copy(
                            ...
                            infoMessage = "Sign in to sync your buy list",
                        )
                    }
                } else {
    ...
    override fun onUiEvent(event: BuyListUiEvent) {
        when (event) {
            is BuyListUiEvent.CollectionSearchChanged -> {
                updateState { state ->
            ...
            is BuyListUiEvent.CollectionCardClicked -> {
                updateState { it.copy(selectedCollectionEntryId = event.entryId) }
            }

            BuyListUiEvent.EnterAddModeClicked -> {
            ...
            BuyListUiEvent.ExitAddModeClicked -> {
                updateState {
                    it.copy(
                        isAddMode = false,
                        editingEntryId = null,
            ...
            is BuyListUiEvent.EditEntryClicked -> {
            ...
            is BuyListUiEvent.RemoveEntryClicked -> {
            ...
            is BuyListUiEvent.AddSearchChanged -> {
                updateState { it.copy(addSearchQuery = event.value) }
            }

            BuyListUiEvent.AddSearchSubmitted -> {
                loadAddResults()
            }

            is BuyListUiEvent.AddResultClicked -> {
            ...
            BuyListUiEvent.ConfirmAddClicked -> {
                val before = state.value.data.collectionEntries
                updateState { state ->
            ...
            BuyListUiEvent.DismissPinRecommendationClicked -> {
                updateState { it.copy(showPinRecommendationDialog = false) }
            }

            is BuyListUiEvent.CurrentLocationResolved -> {
                savePinFromCurrentLocation(event.coordinate)
            }

            BuyListUiEvent.ImportSampleClicked -> {
                val before = state.value.data.collectionEntries
                updateState { state ->
                    val sample = listOf(
                        CollectionCardEntry(
    ...
    private fun persistEntries() {
        ...
        if (signature == lastPersistedSignature) {
            println("TradeBE: buy list persist skipped (unchanged)")
            return
        }
